    ...
    def __init__(
        ...
        *,
        ...
        if not api_key or api_key.startswith("sk-placeholder"):
            raise LLMError(
                "LiteLLMClient: api_key 未配置或为占位符，请在 .env 中设置 LITELLM_API_KEY"
            )
    ...
    async def complete_json(
        ...
        *,
        ...
        try:
            ...
            raise LLMError(
                f"LiteLLM 响应结构异常：缺少 choices[0].message.content；body={body}"
            ) from e
        ...
        try:
            ...
            raise LLMError(f"LiteLLM 响应不符合 {response_model.__name__} schema: {e}") from e
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if response.status_code == 429 or response.status_code >= 500:
                    ...
                    continue
                raise LLMError(
                    f"LiteLLM HTTP {response.status_code} (不可重试): {response.text[:500]}"
                )
            except httpx.RequestError as e:
                wait = 2 ** (attempt - 1)
                logger.warning(
                    "litellm_network_retry",
                    attempt=attempt,
                    error=str(e),
                    wait_seconds=wait,
                )
                last_err = LLMError(f"LiteLLM 网络错误: {e}")
                if attempt < self.max_retries:
                    await asyncio.sleep(wait)
        raise last_err or LLMError("LiteLLM 重试用尽（未知原因）")
